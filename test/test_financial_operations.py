...
class TestFinOp(unittest.TestCase):
    def setUp(self):
        self.exp1 = FinOp('Rent', '2022-01-01-00-00', 'expense', 'Housing', 1000.0)
        self.exp2 = FinOp('Groceries', '2022-01-02-00-00', 'expense', 'Food', 200.0)
        self.inc1 = FinOp('Salary', '2022-01-01-00-00', 'income', 'Job', 3000.0)

    def test_name_type_check(self):
        with self.assertRaises(ValueError):
            FinOp(1, '2022-01-01-00-00', 'expense', 'Housing', 1000.0)
    ...
    def test_op_type_type_check(self):
        with self.assertRaises(ValueError):
            FinOp('Rent', '2022-01-01-00-00', 1.0, 'Housing', 1000.0)

    def test_category_type_check(self):
        with self.assertRaises(ValueError):
            FinOp('Rent', '2022-01-01-00-00', 'expense', 1.0, 1000.0)

    def test_value_type_check(self):
        with self.assertRaises(ValueError):
            FinOp('Rent', '2022-01-01-00-00', 'expense', 'Housing', "00")

    def test_value_value_check(self):
        # check if value <= 0 raises an error
        with self.assertRaises(ValueError):
            FinOp('Rent', '2022-01-01-00-00', 'expense', 'Housing', 0.0)        # check if value <= 0 raises an error
        with self.assertRaises(ValueError):
            FinOp('Rent', '2022-01-01-00-00', 'expense', 'Housing', -1000.0)
